...
/**
 * С помощью классов, создайте магазин, в котором есть товары. 
 * Эти товары кладутся в корзину, формируется заказ.
 * Также есть пользователи. При checkout (оформлении заказа) всё,
 * что лежит в корзине, становится заказом.
 * В корзину и заказ нельзя добавить товар,
 * у которого количество ниже 0.
 * К корзине и заказу привязывается пользователь.
 * У заказа есть служба доставки, а также при формировании заказа
 * могут быть применены различные скидки (процентная,
 * скидка на доставку, товар по акции).
 */
...
fclose($editedHtmlCode);
fclose($cuttedHtmlCode);
